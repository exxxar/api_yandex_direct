...
class YandexDirect extends Command
{
    ...
    public function handle()
    {
            ...
            foreach ($contents as $c) {
                ...
                if (trim($c)){
                    ...
                    try{
                        ...
                        $this->log->error("Ну удалось данные в таблицу 'AuctionBids': ".$e->getMessage());
                    }

                    try{
                        ContextCoverage::insert([
                            'Keywords_id'=>$id
                        ]);
                    }catch (\Exception $e){
                        $this->log->error("Ну удалось данные в таблицу 'ContextCoverage': ".$e->getMessage());
                    }

                    try{
                        CompetitorsBids::insert([
                            'Keywords_id'=>$id
                        ]);
                    }catch (\Exception $e){
                        $this->log->error("Ну удалось данные в таблицу 'CompetitorsBids': ".$e->getMessage());
                    }
                }
            }
        }
        ...
        foreach($words as $w){
            ...
            //тут нужно выбирать слова, чьи сроки check_date+1 месяц меньше текущей даты
            if ($yandex_type==0){
            ...
            foreach ($reports as $w){
                ...
                foreach ($report as $wr) {
                    foreach ($wr->getSearchedWith() as $sw) {
                        ...
                        try{
                            ...
                            $this->log->error("Не удалось данные в таблицу 'AuctionBids': ".$e->getMessage());
                        }
                        ...
                        try{
                            ...
                            $this->log->error("Не удалось данные в таблицу 'ContextCoverage': ".$e->getMessage());
                        }
                        ...
                        try{
                            ...
                            $this->log->error("Не удалось данные в таблицу 'CompetitorsBids': ".$e->getMessage());
                        }
                ...
                sleep(10);
                //после выбора слов из репорта и удалении репорта, создаем комапнию
                $idCampaign = $this->api->addCampain("test company")
                    ->getAddResults()[0]
                    ->getId();
                sleep(10);
                ...
                if (empty($buf)) {
                    error_log("Содержимое массива ключевых слов отсутствует!");
                    continue;
                }
